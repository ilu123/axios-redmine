...
Redmine.prototype = {
  ...
  set basicAuth (ba) {
    this.config.basicAuth = ba
  },
  get axiosConfig () {
    return this.config.axiosConfig
  },
  set axiosConfig (ac) {
    this.config.axiosConfig = ac
  }
}
...
Redmine.prototype.request = function (method, path, params) {
  ...
  if (this.config.axiosConfig) {
    opts = Object.assign(opts, this.config.axiosConfig)
  }
  ...
  if (this.basicAuth) {
    opts.headers['Authorization'] = this.config.basicAuth
  }
